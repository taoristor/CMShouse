...
<%if(IsSearch==0){conn.Open();%><ul class="article_pic">
<%
string url="";
...
url=Detail_Url(dr);
%>
<li title="<%=Server.HtmlEncode(dr["title"].ToString())%>">
<a href="<%=url%>" target="<%=Target%>" class="pic"><img src="<%=dr["titlepic"]%>" border="0"></a>
<a href="<%=url%>" target="<%=Target%>" class="title"><%=SubStr(dr["title"].ToString(),Title_Num,true)%></a>
</li>
<%
 }
%>
