...
<li class="info">
Release date:<%=((DateTime)dr["thedate"]).ToString("yyyy-MM-dd")%>&nbsp;&nbsp;
<%=dr["pa_autor"].ToString()==""?"":"&nbsp;Author:"+dr["pa_autor"].ToString()%>
Views:<span id="clicks"></span>&nbsp;&nbsp;<%=dr["pa_source"].ToString()==""?"":"&nbsp;Source:"+dr["pa_source"].ToString()%>&nbsp;&nbsp;Font size:[&nbsp;<a href="javascript:FontZoom('16px','Content')">Big</a> <a href="javascript:FontZoom('14px','Content')">In the</a>  <a href="javascript:FontZoom('12px','Content')">Little</a>]
</li>
</ul>
<%//图片组开始
if(News_Pic>0)
...
<li><%=_Previous==""?"":"An article on:"+_Previous%></li>
<li><%=_Next==""?"":"Next:"+_Next%></li>
</ul><%conn.Close();%>
