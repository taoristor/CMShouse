...
<%if(IsSearch==0){conn.Open();%><ul class="pic_list">
<% 
...
%><li><a href="<%=Detail_Url(dr)%>" target="<%=Target%>"><img src="<%=pic%>"></a><span class="title"><a href="<%=Detail_Url(dr)%>" target="<%=Target%>"><%=SubStr(dr["title"].ToString(),Title_Num,true)%></a></span></li><%
 }
%></ul><%conn.Close();}
if(PageCount>1)
{
