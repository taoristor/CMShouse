...
<span style="display:block;font-size:13px;line-height:22px;">产品型号：<%=SubStr(dr["pa_xinghao"].ToString(),50,true)%></span>
<%if(Price!="" && Price!="0"){%><span style="display:block;font-size:13px;line-height:30px;">产品价格：<%=Price%>元 &nbsp;<a href="javascript:ordercart('<%=Site_Id%>','product',<%=Id%>)" style="color:#ff0000">[在线订购]</a></span><%}%>
