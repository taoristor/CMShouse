...
<tr id='tr_product_pa_jscs'><td class='tdhead'>技术参数</td><td><textarea name='pa_jscs' id='pa_jscs' style="width:100%;height:300px"   ><%if(post=="add"){Response.Write("");}else{Response.Write(r("pa_jscs"));}%></textarea><script charset="utf-8" src="/e/incs/kindeditor/kindeditor.js" type="text/javascript"></script><script type="text/javascript">var KE_pa_jscs;KindEditor.ready(function(K) {KE_pa_jscs= K.create("#pa_jscs",{uploadJson :kindeditor_uploadJson,fileManagerJson :kindeditor_fileManagerJson,allowImageUpload:<%=Editor_ImageUpload%>,allowFlashUpload:<%=Editor_FlashUpload%>,allowMediaUpload:<%=Editor_FlashUpload%>,allowFileUpload:<%=Editor_AttachmentUpload%>,allowFileManager :false,items :kindeditor_NormalItems,filterMode :false,extraFileUploadParams:{siteid:"<%=SiteId%>"}});});</script><span id="pa_jscs_tip"></span></td></tr>
<tr id='tr_product_content'><td class='tdhead'>产品介绍</td><td><textarea name='content' id='content' style="width:100%;height:300px"   ><%if(post=="add"){Response.Write("");}else{Response.Write(r("content"));}%></textarea><script type="text/javascript">var KE_content;KindEditor.ready(function(K) {KE_content= K.create("#content",{uploadJson :kindeditor_uploadJson,fileManagerJson :kindeditor_fileManagerJson,allowImageUpload:<%=Editor_ImageUpload%>,allowFlashUpload:<%=Editor_FlashUpload%>,allowMediaUpload:<%=Editor_FlashUpload%>,allowFileUpload:<%=Editor_AttachmentUpload%>,allowFileManager :false,items :kindeditor_NormalItems,filterMode :false,extraFileUploadParams:{siteid:"<%=SiteId%>"}});});</script><span id="content_tip"></span></td></tr>
...
Iframe_Submit("iframe_pa_images","1")
KE_pa_jscs.sync();
KE_content.sync();
return true;
}
